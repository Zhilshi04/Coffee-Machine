...
public class CoffeeMachineTextUI extends CoffeeMachine{
    ...
    public CoffeeMachineTextUI(int water, int milk, int beans, int cups, int cash) {
    ...
    public void doBuyCoffee() {
        ...
        if (option.equals("back")) {
            ...
            CoffeeTypes coffee = CoffeeTypes.values()[coffeeType - 1];

            switch (coffee) { //ดูเงื่อนไขจาก values ของ Enum
                case ESPRESSO, CAPPUCCINO, LATTE -> { // ถ้าตรงกับ ESPRESSO หรือ CAPPUCCINO หรือ LATTE
                    String coffeeMessage = super.makeCoffee(coffee); //ให้เรียก super.makeCoffee(values ของ Enum); และค่าที่ return จาก makeCoffee => coffeeMessage
    ...
    public static void main(String[] args) {
        CoffeeMachineTextUI machineTextUI = new CoffeeMachineTextUI(400, 540, 120, 9, 550);
        boolean done = false;
        do {
